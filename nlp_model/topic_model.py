...
class TopicClassifer():
    def __init__(self):
        self.lda = open('./model_train/lda_model.pkl', 'rb')
        self.tf = open('./model_train/tf_vectorizer.pkl', 'rb')
        
    def get_topic_keywords(self, lda_model, vectorizer, text, num_keywords):
    ...
    def topic_model(self, text):
        # Load the LDA model
        lda_model_loaded = pickle.load(self.lda)

        # Load the vectorizer
        tf_vectorizer_loaded = pickle.load(self.tf)
            
        dominant_topic, keywords = self.get_topic_keywords(lda_model_loaded, tf_vectorizer_loaded, text, 10)
